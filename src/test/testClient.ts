...
async function testMcpServer() {
  ...
  try {
    ...
    const randomCatResponse = await fetch(serverUrl, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "mcp-session-id": sessionId
    ...
    const breedsResponse = await fetch(serverUrl, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "mcp-session-id": sessionId
    ...
    const breedsResult = await breedsResponse.json() as any;
    console.log("Breeds Result (truncated):", 
      JSON.stringify({
        id: breedsResult.id,
